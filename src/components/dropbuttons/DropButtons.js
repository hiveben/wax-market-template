                    ...
                    : <div className={cn(
                        ...
                        <Button
                            className={cn(
                                ...
                                {'bg-primary text-secondary': dropReady && (!accountStats || (accountLimit - accountStats.counter) > 0)},
                                {'bg-gray-600 text-neutral': !dropReady || (accountStats && (accountLimit - accountStats.counter) <= 0)},
                            )}
                            onClick={userName ? claim : ual.showModal}
                            disabled={!dropReady || (accountStats && (accountLimit - accountStats.counter) <= 0)}
                        >
                            {dropTimeLeft && !dropReady ? dropTimeLeft : (userName ? (free ? 'Claim' : 'Purchase') : 'Login')}
                        </Button>
                    </div>}
        </div> }
        {dropEnded && <div
